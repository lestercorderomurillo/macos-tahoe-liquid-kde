...
def test_installer_package(repo):
    assert (repo / "installer/__init__.py").is_file()
    assert (repo / "installer/cli.py").is_file()
    assert (repo / "installer/theme_switch.py").is_file()
    assert (repo / "installer/transparency.py").is_file()


def test_theme_switch_python(repo):
    text = (repo / "installer/theme_switch.py").read_text()
...
def test_acrylic_step_sets_key(repo, key):
    text = (repo / "installer/steps/acrylic_glass.py").read_text()
    assert key in text


def test_globalmenu_step_cleans_old(repo):
    text = (repo / "installer/steps/globalmenu.py").read_text()
...
def test_cli_feature_lists(repo):
    text = (repo / "installer/cli.py").read_text()
...
def test_nautilus_step(repo):
    text = (repo / "installer/steps/nautilus.py").read_text()
...
def test_portals_step(repo):
    text = (repo / "installer/steps/portals.py").read_text()
...
def test_transparency_python(repo):
    p = repo / "installer/transparency.py"
    text = p.read_text()
...
def test_install_helpers_have_reinstall(repo):
    text = (repo / "installer/log.py").read_text()
