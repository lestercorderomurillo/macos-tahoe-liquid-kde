...
install() {
  ...
  if sudo cp "$effect_so" "${dest_effect}.tmp" && sudo mv -f "${dest_effect}.tmp" "$dest_effect" && \
    ...
    kwriteconfig6 --file kwinrc --group "$grp" --key BlurStrength       2.0  2>/dev/null || true
    ...
    kwriteconfig6 --file kwinrc --group "$grp" --key HighlightStrength   0.40 2>/dev/null || true
